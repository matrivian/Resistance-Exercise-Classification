...
class ResistanceClassification:
    ...
    def plot(self, rawMat, dataMat, label, filename, flag):
        ...
        if flag == 0:
            ...
            sx, = plt.plot(rawMat[0,:], rawMat[1,:], color='0.25', linestyle=':', marker='o', markersize=8, markevery=(len(rawMat[0,:])-1)/3, linewidth=1.5) 
            sy, = plt.plot(rawMat[0,:], rawMat[2,:], color='0.35', linestyle='-', marker='v', markersize=8, markevery=(len(rawMat[0,:])-1)/3, linewidth=1.5)
            sz, = plt.plot(rawMat[0,:], rawMat[3,:], 'k-', marker='d', markersize=8, markevery=(len(rawMat[0,:])-1)/3, linewidth=1.5)
            ...
            sx, = plt.plot(dataMat[0,:], dataMat[1,:], color='0.25', linestyle=':', marker='o', markersize=8, markevery=(len(dataMat[1,:])-1)/3, linewidth=1.5)
            sy, = plt.plot(dataMat[0,:], dataMat[2,:], color='0.35', linestyle='-', marker='v', markersize=8, markevery=(len(dataMat[1,:])-1)/3, linewidth=1.5)
            sz, = plt.plot(dataMat[0,:], dataMat[3,:], 'k-', marker='d', markersize=8, markevery=(len(dataMat[1,:])-1)/3, linewidth=1.5)
            plt.plot(timeList, peakList, 'k*', markersize=12)
            plt.legend([sx, sy, sz], ['X', 'Y', 'Z'], bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0., prop={'size':8})
            plt.grid(True)
        elif flag == 1:
            plt.figure()
            ...
            plt.xlabel('Time (s)')
            plt.ylabel('Filtered Accelerometer Data (g)')
            sx, = plt.plot(dataMat[0,:], dataMat[1,:], 'k:', marker='o', markevery=(len(dataMat[1,:])-1)/3)
            sy, = plt.plot(dataMat[0,:], dataMat[2,:], color='0.85', linestyle='-', marker='v', markevery=(len(dataMat[1,:])-1)/3)
            sz, = plt.plot(dataMat[0,:], dataMat[3,:], 'k-', marker='d', markevery=(len(dataMat[1,:])-1)/3)
            plt.plot(timeList, peakList, 'k*')
            plt.legend([sx, sy, sz], ['X', 'Y', 'Z'], bbox_to_anchor=(1.01, 1), loc=2, borderaxespad=0., prop={'size':8})
            plt.grid(True)
        plt.savefig(filename + self.getClassname(label) + '.jpg')      # save figure
        return len(peakList)
